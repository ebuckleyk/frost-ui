...
function MessageScrollerViewport({
  ...
  return (
    <MessageScrollerPrimitive.Viewport
      ...
      className={cn(
        `
          scrollbar-thin scrollbar-gutter-stable size-full min-h-0
          min-w-0 scroll-fade-b overflow-y-auto
          overscroll-contain
          contain-content
        `,
        className,
...
function MessageScrollerButton({
  ...
  return (
    <MessageScrollerPrimitive.Button
      ...
      className={cn(
        `
          absolute start-1/2 -translate-x-1/2 border-border bg-background
          text-foreground transition-[translate,scale,opacity] duration-200
          hover:bg-muted hover:text-foreground data-[active=false]:pointer-events-none
          data-[active=false]:scale-95 data-[active=false]:opacity-0
          data-[active=false]:duration-400 data-[active=true]:translate-y-0
