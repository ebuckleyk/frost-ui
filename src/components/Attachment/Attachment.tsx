...
const attachmentVariants = cva(
  `
    ...
    rounded-xl border border-(--glass-edge) bg-card/70 text-card-foreground
    saturate-(--glass-saturation) backdrop-blur-(--glass-blur)
    transition-colors focus-within:ring-1 focus-within:ring-ring/50
...
const attachmentMediaVariants = cva(
  `
    relative flex aspect-square w-10 shrink-0 items-center justify-center overflow-hidden
    rounded-lg border border-(--glass-edge-soft) bg-muted/80 text-foreground
    shadow-[inset_0_1px_0_var(--glass-highlight-soft)]
    group-data-[orientation=vertical]/attachment:w-full
    group-data-[size=sm]/attachment:w-8 group-data-[size=xs]/attachment:w-7
    group-data-[size=xs]/attachment:rounded-md group-data-[state=error]/attachment:bg-destructive/10
    ...
    variants: {
      variant: {
        ...
        image: `
          ...
          group-data-[state=idle]/attachment:opacity-100 *:[img]:aspect-square
          *:[img]:w-full *:[img]:object-cover
        `,
      },
    },
    defaultVariants: {
...
function AttachmentGroup({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        `
          scrollbar-none flex min-w-0 scroll-fade-x snap-x snap-mandatory scroll-px-1
          gap-3 overflow-x-auto overscroll-x-contain py-1
